...
- 実行コスト: ケースは 6 件・各1回生成 + 1回 judge。リトライは API エラー時のみ
  1回(品質不合格のリトライはしない — 揺れは閾値側で吸収する)。429(レート制限)は
  レスポンスの retryDelay を読み取り、最大60秒待ってから再試行する(即時リトライは
  同じレート窓を踏むだけ — nightly 実運用で実証)。
- **judge は生成と別モデル**: `GEMINI_JUDGE_MODEL ?? "gemini-flash-lite-latest"`。
  無料枠のクォータはモデル別(20リクエスト/日)のため、生成(+llm-smoke)と judge を
  同一モデルに載せると1晩で枯渇する。短文 JSON 採点は flash-lite で足りる。
