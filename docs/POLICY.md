...
検証系(VRT・lint ルール・ゲート)を新設・変更するときは、**意図的な違反や変化を
1つ作って検出されること(赤くなること)を確認してから**導入する。検出できない
検証系は「通っている」ことが何も保証しない(VRT の threshold 既定値はダークテーマの
面変化を全て沈黙で通していた)。
...
- 一覧・盤面への可視アサーションを DB 総量に依存させない。行の存在は自テストが作った
  アンカーで検証し、仮想化されたリストで「見えること」まで要るときはフィルタ・検索で
  絞り込むか表示データを固定する(可視窓の外に押し出された行は DOM に存在しない)
