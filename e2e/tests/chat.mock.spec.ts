...
test.describe("chat with mocked API @feature-chat", () => {
  test(
    "sends a message and renders the streamed reply @smoke",
    {
      annotation: {
        type: "description",
        description: "メッセージ送信からストリーミング応答の全文表示までの主経路を検証",
      },
    },
    async ({ page }, testInfo) => {
  ...
  test(
    "shows an error banner when the API returns 500",
    {
      annotation: {
        type: "description",
        description:
          "APIが500を返したときにエラーバナーが出てアシスタント応答が残らず再送可能なことを検証",
      },
    },
    async ({ page }, testInfo) => {
  ...
  test(
    "stop cancels a streaming response @pinned",
    {
      annotation: {
        type: "description",
        description:
          "ストリーミング中のStop操作で応答が即座に中断されエラー扱いにならないことを検証",
      },
    },
    async ({ page }, testInfo) => {
      await page.route("**/api/chat", async (route) => {
  ...
  test(
    "keeps partial text and shows an error when the stream fails mid-way",
    {
      annotation: {
        type: "description",
        description:
          "ストリーム途中で失敗した場合に部分テキストを保持しつつエラーバナーを表示することを検証",
      },
    },
    async ({ page }, testInfo) => {
      await page.route("**/api/chat", (route) =>
