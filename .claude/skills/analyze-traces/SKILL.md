...
このスキルはユーザーに頼まれたときだけでなく、以下のタイミングで**能動的に**回す:

1. **機能マイルストーンのクローズ時**(dev-cycle Phase 3 の後): `pnpm e2e:perf` →
   解析 → 前回レポートと比較。新機能が遅い関数・異常な呼び出し回数
   (N+1、無駄な再フェッチ)を持ち込んでいないかをマージ直後に確認する
2. **nightly perf レーンの成果物が出た後のセッション**: 直近の perf-traces を
   ダウンロードして解析し、悪化検知(2倍閾値)だけでも確認する

観測して異常が無ければ「健全」と一行記録して終了してよい。目的は常時監視ではなく、
**劣化を持ち込んだ変更が特定できるうちに気づく**こと。
